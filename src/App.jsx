...
export default function App() {
  ...
  async function handleSignIn() {
    ...
    setIsSigningIn(true)
    try { await signInWithGoogle() } catch (err) { console.error('Sign-in failed:', err) } finally { setIsSigningIn(false) }
  }
        ...
        <div className="flex items-center gap-2">
          ...
          {showAvatar && (
            <div className="relative" ref={menuRef}>
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-1 w-56 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50 animate-dropdown" role="menu">
  ...
  function renderInput() {
    ...
    return (
      <div className="flex-shrink-0 bg-[#f2f2f2] px-4 pt-2" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
        ...
        <div
          className={`chat-input-hint hidden md:block max-w-2xl mx-auto text-right text-[11px] font-sans text-gray-400 transition-opacity duration-200 mt-1.5 pr-1 ${
            inputFocused || hasText ? 'opacity-100' : 'opacity-0'
          }`}
          aria-hidden="true"
        >
          <kbd>Enter</kbd> to send · <kbd>Shift</kbd> + <kbd>Enter</kbd> for new line · <kbd>Esc</kbd> to clear
        </div>
        {!user && (
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="max-w-md w-full text-center animate-fade-in">
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-amber-600 text-white font-sans text-base hover:bg-amber-700 mb-3 animate-fade-in-up stagger-2 btn-press transition-opacity${isSigningIn ? ' opacity-80 cursor-wait' : ''} disabled:opacity-80 disabled:cursor-wait`}
            >
              {isSigningIn ? (
            ...
            <button
              ...
              disabled={isSigningIn}
              className="w-full px-6 py-3 rounded-xl border border-gray-300 text-gray-600 font-sans text-base hover:bg-white hover:border-gray-400 transition-colors mb-6 animate-fade-in-up stagger-3 card-hover disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              onClick={handleSignIn}
              disabled={isSigningIn}
              className="text-sm font-sans text-gray-400 hover:text-amber-600 transition-colors underline underline-offset-2 animate-fade-in-up stagger-4 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center justify-center gap-1.5"
            >
              {isSigningIn && (
              ...
              {isSigningIn ? 'Signing in…' : 'Already have an account? Log in'}
            </button>
          </div>
        </div>
      ...
      <div className="h-dvh flex flex-col bg-[#f2f2f2]">
        ...
        <div className="flex-1 flex items-center justify-center px-4 overflow-y-auto">
          <div className="max-w-lg w-full py-8">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-4 text-left">
                  ...
                  {importError && (
                    <div className="mt-3 flex items-start gap-2 px-3 py-2.5 bg-red-50 rounded-lg">
                      ...
                      <p className="text-sm text-red-700 font-sans leading-snug">{importError}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {onboardingStep === 'done' && (
              <div className="text-center animate-fade-in">
                <div className="mx-auto mb-4 w-16 h-16 rounded-full bg-green-50 flex items-center justify-center animate-fade-in-scale">
